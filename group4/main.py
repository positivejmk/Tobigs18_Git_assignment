...
def greeting():
    print("< Tobigs 18 정규세션 git >")
    print("투빅스 18기 여러분들 환영합니다.")
    print("여기는 정규세션 과제 월드입니다.")
    time.sleep(WAITTIME + 1) # 연출을 위해 1초 추가
    print("그리고 저는 당신의 과제 달성을 도와줄 과제 요정입니다 ^^ ")
    print("과제 중 막히는 부분이 있으면 '17기 김정민'에게 언제든지 카톡 주세요~ ")
    time.sleep(WAITTIME)
    ...
    for i in range(1, NUM_PLAYERS): # 플레이어 5명을 players 배열에 담슴니다.
        try:
            with open("player" + str(i) + "/profile.json", "r", encoding='utf-8') as f:
                data = json.load(f)
                if(data["닉네임"] == "[여기에 별명을 적어주세요]" or data["한마디"] == "[투빅스 18기에 임하는 각오 한 마디 적어 주세요!]"): # 모두의 파일이 작성되지 않음
                    time.p(WAITTIME)
                    print("파일을 작성한 뒤에는 팀장의 레포지토리에 'git commit'하는 것 잊지 마세요!")
                    time.sleesleep(WAITTIME)
...
def clear():
    ...
    time.sleep(WAITTIME)
    print("이 기세로 투빅스 18기 컨퍼런스 까지 파이팅입니다!")
    time.sleep(WAITTIME)
